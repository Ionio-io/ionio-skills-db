...
- **Vercel.** `vercel.json` serves the dashboard statically and routes `/api/*` and `/mcp` to one function (`api/index.js` → `packages/server/src/bin/vercel.ts`, built first). `/api/server` derives `mcpUrl` from the request's host (`x-forwarded-host`/`-proto`), so it always matches the domain; don't reintroduce a fixed URL. Hosted mode sends `stdio: null, live: false`: the dashboard hides stdio setups and skips `/api/events` (which answers 204). Content reaches the function through `includeFiles`; check a change there with `npx vercel build` and the `.vc-config.json` `filePathMap`. The install is `npm ci --include=dev` because the build needs devDependencies (tsc, vite) and Vercel omits them when `NODE_ENV=production`.
