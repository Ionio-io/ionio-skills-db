...
Skills for Ionio's outbound motion to CEOs of mid-market e-commerce and retail SaaS companies (post-purchase and returns), plus coaching on sales, scoping, and kickoff calls.
...
- `mannan-call-breakdown/references/pattern-ledger.md`: Mannan's running record of strengths, recurring leaks, growth, and the root pattern. It's read before every breakdown. **Update it after each review.**

## Pipeline

```
prospect research → loom-email-copy → send Loom → outbound-crm-update → call → mannan-call-breakdown
```

## Dependencies

- **outbound-crm-update** needs the Notion connector, with access to the Outbound Prospect CRM database. The database IDs are hard-coded in the skill.
